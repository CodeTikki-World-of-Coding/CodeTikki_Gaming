...
if ($user_id !== null && !empty($user_id)) {
    try {
        ...
        if ($typeResult) {
            ...
            $stmtInsert = $pdo->prepare("INSERT INTO QuestionBank (QuestionTitle, Pseudocode, Option1, Option2, Option3, Option4, CorrectAnswer, Level, UserID, QuestionType, CreateDate) 
                                   VALUES (:questionTitle, :pseudocode, :option1, :option2, :option3, :option4, :correctAnswer, :level, :userID, :questionType, :createDate)");
            ...
            $stmtInsert->bindParam(':createDate', $currentDate);
            ...
            foreach ($formData as $key => $value) {
                if (strpos($key, 'cloned_question_') !== false) {
                    $parts = explode('_', $key);
                    $index = end($parts);
                    $stmtClone = $pdo->prepare("INSERT INTO QuestionBank (QuestionTitle, Pseudocode, Option1, Option2, Option3, Option4, CorrectAnswer, Level, UserID, QuestionType, CreateDate) 
                                       VALUES (:questionTitle, :pseudocode, :option1, :option2, :option3, :option4, :correctAnswer, :level, :userID, :questionType, :createDate)");
                    if (!empty($formData['cloned_question_' . $index])) {
                        ...
                        $stmtClone->bindParam(':createDate', $currentDate);
                        $stmtClone->execute();
                    }
                }
            }
